...
export class StairwayControl extends PIXI.Container {
  ...
  async draw () {
    ...
    // scene label
    this.sceneLabel = this.sceneLabel || this.addChild(new foundry.canvas.containers.PreciseText(this.stairway.sceneLabel, this.stairway.controlIcon.sceneLabelTextStyle))
    ...
    // label
    this.label = this.label || this.addChild(new foundry.canvas.containers.PreciseText(this.stairway.label, this.stairway.labelTextStyle))
    ...
    this.lockIcon.texture = await foundry.canvas.loadTexture('icons/svg/padlock.svg')
  ...
  async _onMouseDown (event) {
    ...
    if (targetSceneId !== null) {
      ...
      // teleport/move tokens within scene (update position and level)
      ...
      const changingLevel = targetLevelId !== currentLevelId
      // only animate within the same level; cross-level is always an instant teleport
      const animate = this.stairway.document.animate === true && !changingLevel
      ...
      // if animation is in progress walls will always block movement, annoyingly
      // if we are not animating, then we use the isPaste flag to prevent walls blocking
      await canvas.scene.updateEmbeddedDocuments(foundry.canvas.placeables.Token.embeddedName, tokenData, { animate, isPaste: !animate })

      // switch level view if needed
      if (canvas.level?.id !== targetLevelId) {
        await canvas.scene.view({ level: targetLevelId })

        // re-select tokens after level switch (canvas redraw clears selection)
        const movedTokens = canvas.tokens.placeables.filter(t => selectedTokenIds.indexOf(t.id) >= 0)
    ...
    // GM pan to target
    if (selectedTokens.length === 0) {
      const panLevelId = targetData.level ?? foundry.documents.BaseScene.metadata.defaultLevelId
      if (targetSceneId !== null) {
        await targetScene.view({ level: panLevelId })
      } else if (canvas.level?.id !== panLevelId) {
        await canvas.scene.view({ level: panLevelId })
      }
