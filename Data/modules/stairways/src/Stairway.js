...
export class Stairway extends PlaceableObject {
  ...
  get target () {
    ...
    if (targetScene) {
      ...
      const others = (targetScene.flags.stairways?.data ?? []).filter((other) => this.name === other.name)
  ...
  _applyRenderFlags (flags) {
    ...
    if (flags.refreshState) this._refreshStairwayState()
  }
  ...
  _refreshPosition () {
    ...
    if (renderState === 'master' || renderState === 'move') {
      ...
      this.otherPlaceable.renderFlags.set({ refreshPosition: true })
    }
  }

  _refreshStairwayState () {
    // update icon tint
